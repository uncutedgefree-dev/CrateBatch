...
const LibraryDashboard: React.FC<LibraryDashboardProps> = ({ 
  ...
  // CLEANUP: Filter out "0" and empty years from timeline view
  const activeTimeline = yearDistribution.filter(y => y.name !== "0" && y.name !== "");
           ...
           <div className="flex-1 w-full h-full min-h-0 relative z-10 text-xs font-mono">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={situationDistribution}>
                  ...
                  <PolarAngleAxis dataKey="name" tick={{ fill: '#888', fontSize: 10, dy: 3 }} />
                  <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={false} axisLine={false} />
                  <Radar name="Situations" dataKey="value" stroke="#00f3ff" fill="#00f3ff" fillOpacity={0.4} />
        ...
        <div className="bg-dj-panel border border-dj-border rounded-sm p-6 flex flex-col relative overflow-hidden shadow-lg">
          ...
          <div className="flex-1 w-full h-full min-h-0 relative z-10 flex items-center justify-center">
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={keyDistribution.major} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={80} startAngle={90} endAngle={-270} stroke="none" onClick={(data) => onFilter('key', data.name)} className="cursor-pointer outline-none">
                    {keyDistribution.major.map((_, index) => <Cell key={`maj-${index}`} fill={CAMELOT_COLORS_MAJOR[index]} />)}
                  </Pie>
                  <Pie data={keyDistribution.minor} dataKey="value" cx="50%" cy="50%" innerRadius={40} outerRadius={60} startAngle={90} endAngle={-270} stroke="none" onClick={(data) => onFilter('key', data.name)} className="cursor-pointer outline-none">
                    {keyDistribution.minor.map((_, index) => <Cell key={`min-${index}`} fill={CAMELOT_COLORS_MINOR[index]} />)}
                  </Pie>
                  <Tooltip content={<CustomTooltip />} />
                </PieChart>
              </ResponsiveContainer>
